...
module.exports.run = async (ws, msg) => {
        ...
        await ws.sendData(
          {
            ...
            duration: 15000,
            target: "#room",
            html: `You are currently banned from the site for forever.<br>Reason: ${user[0].ban.reason}<br>Your _ID: ${user[0].p._id}<br>${config.punishment}`,
            title: "Notice",
          },
          true
        );
        ...
        await ws.sendData(
          {
            ...
            duration: 15000,
            target: "#room",
            html: `You are currently banned from the site for ${fun.fun.mstotime(
              user[0].ban.ends - Date.now()
            )}.<br>Reason: ${user[0].ban.reason}<br>Your _ID: ${
              user[0].p._id
            }<br>${config.punishment}`,
            title: "Notice",
          },
          true
        );
    ...
    if (!user[0].mute.permanent && Date.now() > user[0].mute.ends) {
      ...
      if (user[0].mute.permanent) {
        ws.sendData({
          ...
          duration: 15000,
          target: "#room",
          html: `You are currently ${
            user[0].mute.type === "all" ? "note and chat" : user[0].mute.type
          } muted on the site for forever.<br>Reason: ${
            user[0].mute.reason
          }<br>Your _ID: ${user[0].p._id}<br>${config.punishment}`,
          title: "Notice",
        });
      } else
        ws.sendData({
          ...
          html: `You are currently ${
            user[0].mute.type === "all" ? "note and chat" : user[0].mute.type
          } muted on the site for ${fun.fun.mstotime(
            user[0].mute.ends - Date.now()
          )}.<br>Reason: ${user[0].mute.reason}<br>Your _ID: ${
            user[0].p._id
          }<br>${config.punishment}`,
          title: "Notice",
        });
  ...
  //console.log(user)
  if (
    connections.filter((a) => a._id === user[0].p._id).length >=
    user[0].q.clients * config.quotas.clients
  ) {
    await ws.sendData(
      {
        m: "notification",
  ...
  [
    ...
    ws.quotas[a] = fun.fun.quota(
      config.quotas[a][0] * user[0].q[a],
      config.quotas[a][1]
    );
  });
  ...
  if (!userset[ws._id])
    userset[ws._id] = fun.fun.quota(
      config.quotas.userset[0] * user[0].q.userset,
      config.quotas.userset[1]
    );
};
